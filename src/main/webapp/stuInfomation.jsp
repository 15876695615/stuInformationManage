...
<body>
    ...
    <table border="1" width="100%" style="text-align: center;">
        ...
        <c:forEach items="${studentList}" var="stuList" varStatus="ids">
            <tr>
                <td>${ids.count}</td><td>${stuList[0].s_name}</td><td>${stuList[0].s_sex}</td>
                <td>${stuList[1].sc_name}</td>
                <td>${stuList[0].s_age}</td><td>${stuList[0].s_mail}</td>
                <td>${stuList[0].s_tel}</td><td>${stuList[0].s_onschool.toString().substring(0,10)}</td>
                <td>${stuList[0].s_status==1?"正常":"休学"}</td>
            </tr>
        </c:forEach>
    </table>
